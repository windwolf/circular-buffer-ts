export class CircularBuffer {
   ...
   */
  public write(data: Uint8Array, allowCover: boolean = true): number {
    let overflow = false;
    let dataOffset = 0;
  ...
  public peek(start: number = 0, length: number = 1): Uint8Array | null {
    const size = this.getSize();
    if (start >= size) {
      return null;
    }
  ...
  public peekOne(offset: number, force: boolean = false): number | null {
    if (force) {
