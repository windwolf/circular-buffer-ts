...
test('new', () => {
    expect(() => { new CircularBuffer(6); }).toThrowError(new Error("Capacity must be a power of two. 6 is not."));
    expect(() => { new CircularBuffer(8); }).not.toThrowError();
});
test('write without cover', () => {
    const cb = new CircularBuffer(8);
